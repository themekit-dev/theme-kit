...
| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `key` | `string` | Storage key used for the selection. |
| `view` | `Window \| undefined` | The `Window` used to access storage and listen for `storage` events. Defaults to the global `window` when present. |
...
| `options` | `SyncFirstRootOptions` | See SyncFirstRootOptions. |
...
| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `options` | `ThemeBroadcastOptions` | The broadcast configuration. |
...
| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `options` | `ThemePersistenceOptions` | The persistence configuration. |
...
| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `options` | `ThemeRuntimeOptions<T>` | Runtime configuration and initial theme definitions. |
...
| `options` | `EmitDiagnosticOptions` | Environment, deduplication and sink overrides. |
...
| `options` | `{ dev?: boolean }` | `dev` overrides environment detection. Intended for tests and for callers that already know which build they are in. |
...
| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `options` | `Partial<ThemeRuntimeOptions<T>>` | The provider's own options. These win, so a local override stays possible. |
...
| `resolveTokenRefs` | `boolean` | Whether to resolve token references in the result. Default `true`. |
...
| `prefersDark` | `boolean` | Whether the visitor prefers dark mode. Used only when `mode` is `"system"`. Default `false`. |
...
| `options` | `ThemeToCSSVariablesOptions` | Emission options (prefix, group filter). |
...
| `options` | `ValidateThemeOptions` | Optional registry used to resolve the theme by name first. |
...
| `options` | `ValidateThemeContrastOptions` | Optional configuration. |
...
- [Scopes](/scoped-theme) — A scope re-resolves tokens for a subtree, optionally from a local theme definition, without mutating the global selection.
- [Transitions](/animation) — Diff-driven, group-aware CSS animations that run on every theme change and can be disabled per change.
...
- [Multi-window sync](/multi-window-sync) — Keep every tab and window on the same selection using BroadcastChannel, SharedWorker or StorageEvent — chosen at runtime.
...
- [Sunrise & sunset](/sunrise-sunset) — Compute solar times for a location (or the browser's time zone) and drive light/dark switching from real sunrise and sunset.
- [Accessibility](/accessibility) — Contrast validation, color-vision-deficiency simulation and ready-made high-contrast/large-text profiles.
...
- [Diagnostics](/reference/diagnostics) — Recoverable problems are reported as structured diagnostics: a stable code, a severity, structured context and a concrete correction. Creation, formatting and emission are separate steps, so the same diagnostic can be logged, collected or escalated without duplicating the logic that produced it.
