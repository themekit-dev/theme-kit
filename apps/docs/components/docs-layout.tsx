...
import { buildPageHeadings, type TocItem } from "../lib/toc";
import { GENERATED_TOC_HEADINGS } from "../lib/generated/toc-headings";
import { useFocusTrap } from "./ui/use-focus-trap";
import { centerActiveSidebarLink } from "../lib/sidebar-scroll";
...
const sections: SidebarSection[] = [
  ...
  {
    heading: "Upgrading",
    links: [{ href: "/migrating-to-2", label: "Migrating to 2.0" }],
  },
  {
    heading: "Themes",
    links: [
  ...
  },
  {
    ...
    links: [
      ...
      { href: "/reference/diagnostics", label: "Diagnostics" },
...
export function DocsLayout({
  ...
  // Keep the rail on the current page's link.
  //
  // The rail lives inside the page tree, not the shared layout, so Next
  // re-mounts it on every client-side navigation and the browser resets its
  // `scrollTop` to 0 — you scrolled down to reach a link, clicked it, and the
  // rail jumped back to the top. Re-centering on the active link is the same
  // thing the pre-paint script does on a full load, so the rail behaves the
  // same either way.
  //
  // `scroll-behavior` is pinned to `auto` for the write: `html:focus-within`
  // sets `scroll-behavior: smooth` and the rail inherits it.
  useLayoutEffect(() => {
  ...
  //
  //   1. `headings` — an explicit prop. A page knows its own headings exactly.
  //   2. `GENERATED_TOC_HEADINGS` — committed data for routes whose headings
  //      come from client components (and, on markdown pages, from a body the
  //      walk cannot descend into). Generated by crawling the routes; see
  //      scripts/docs/generate-toc-headings.mjs for why that is necessary.
  //   3. `collectedItems` — the render-tree walk, which is correct only for
  //      pages whose headings are all literal host elements.
  //
  // Ids are allocated by `buildPageHeadings`, the same routine the client
  // scanner uses, so whichever source wins the rail's hrefs match the headings
  // the browser sees and the rail never resizes on hydration.
  const generated = GENERATED_TOC_HEADINGS[pathname];
  const items =
    headings ??
    (generated ? buildPageHeadings(generated) : collectedItems);

  // Whether the page actually has a TOC. Initialized from the server-side
  ...
  // The sidebar's scroll position is no longer persisted. It used to restore the
  // last offset here (and in the root layout's pre-paint script), which meant a
  // reload returned you to wherever you had scrolled — hiding the active link if
  // you had scrolled away from it. The pre-paint script now centers the active
  // link instead, before the first paint, so there is nothing to restore and
  // nothing that could override it after hydration.
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-6 lg:grid lg:grid-cols-[16rem_minmax(0,1fr)_auto] lg:items-start">
        {/* The sidebar is purely navigation, so it is a <nav> landmark rather
            than an <aside>: screen-reader users need to be able to jump straight
            to "Documentation" instead of tabbing through the page. The links
            already carry `aria-current="page"` for the active route. */}
        <nav
          ref={asideRef}
