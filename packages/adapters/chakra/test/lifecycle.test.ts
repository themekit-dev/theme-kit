...
import { createChakraAdapter } from "../src";
...
describe("chakra adapter lifecycle", () => {
  it("installs, rebuilds on theme change, and clears the snapshot on uninstall", () => {
    const runtime = makeRuntime();

    const adapter = createChakraAdapter();
    const handle = runtime.adapters.use(adapter);

    // 1. install → snapshot is available
    expect(adapter.getSnapshot()).not.toBeNull();

    // 2. theme update → snapshot rebuilds (mode-specific primary changes)
