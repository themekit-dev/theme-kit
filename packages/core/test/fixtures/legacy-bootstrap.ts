/**
 * FROZEN BASELINE — do not edit, do not "fix", do not delete.
 *
 * Verbatim copy of `packages/core/src/bootstrap.ts` as it existed before the
 * shared-bootstrap extraction (git HEAD at the time of the migration), with
 * only the relative import specifiers rewritten to reach `src/` from
 * `test/fixtures/`.
 *
 * It exists so the migration audit can answer one question with evidence
 * instead of assertion: does the extracted shared primitive
 * (`buildBootstrapPlan` + `serializeThemeBootstrapScript`) reproduce the
 * behaviour of the code it replaced?
 *
 * The audit lives in `test/bootstrap-migration.test.ts`. Once the migration is
 * committed and the deltas it documents are accepted, this file may be deleted
 * together with that test — but not before, and not silently.
 */
import type { ThemeDefinition, ThemeMode } from "../../src/model/theme";
import { resolveInitialTheme, resolveSelectionTheme } from "../../src/resolver";
import { themeToCSSVariables, type ThemeToCSSVariablesOptions } from "../../src/css";
